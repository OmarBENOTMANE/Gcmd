...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<ErrorDTO> handleConflict(IllegalNullParamException ex, WebRequest request) {
		...
		return new ResponseEntity<>(ErrorDTO.builder().code(ErrorCode.CODE_1.name()).dateTime(LocalDateTime.now())
				.message(ex.getMessage()).status(HttpStatus.BAD_REQUEST.value()).build(), HttpStatus.BAD_REQUEST);
	}

	@ExceptionHandler(value = { ObjectNotFoundException.class })
	protected ResponseEntity<ErrorDTO> handleConflict(ObjectNotFoundException ex, WebRequest request) {

		return new ResponseEntity<>(ErrorDTO.builder().code(ErrorCode.CODE_2.getCode()).dateTime(LocalDateTime.now())
				.message(ex.getMessage()).status(HttpStatus.NOT_FOUND.value()).build(), HttpStatus.NOT_FOUND);
	}
